...
def parse_args():
    ...
    parser.add_argument('candidates_json', type=str, help='Candidates json mapping from image_id --> candidate.')
...
def main():
    ...
    if args.references_json:
        ...
        print('CLIPScore (Text-Alignment): {:.4f}'.format(np.mean([s['CLIPScore'] for s in scores.values()])))
