...
def chunk_compute_mse(K_target, V_target, W, device, chunk_size=5000):
    ...
    for i in range(num_chunks):
        ...
        loss += F.mse_loss(F.linear(K_target[start_idx:end_idx].to(device), W),
                           V_target[start_idx:end_idx].to(device)) * (
                               end_idx - start_idx)
...
def update_quasi_newton(K_target, V_target, W, iters, device):
    ...
    with torch.no_grad():
        if len(W.shape) == 4:
            loss = torch.norm(F.conv2d(K_target, best_W) - V_target, 2, dim=1)
        else:
            loss = torch.norm(F.linear(K_target, best_W) - V_target, 2, dim=1)
...
def get_text_feature(prompts, tokenizer, text_encoder, device, return_type='category_embedding'):
    ...
    if return_type == 'category_embedding':
        for text in prompts:
            ...
            concept_feature = text_encoder(torch.LongTensor(tokens).reshape(
                1, -1).to(device))[0][:, new_token_position].reshape(-1, 768)
...
def parse_new_concepts(concept_cfg):
    ...
    for model_path in model_paths:
        ...
        if 'unet' in model and len(model['unet']) != 0:
            crosskv_matches = ['attn2.to_k_lora', 'attn2.to_v_lora']
            ...
            spatial_attn_dict = {k: v for k, v in model['unet'].items() if all([x not in k for x in crosskv_matches])}
...
def merge_kv_in_cross_attention(concept_list, optimize_iters, new_concept_cfg, tokenizer, text_encoder, unet,
    ...
    for name, _ in unet.down_blocks.named_parameters():
        if any([x in name for x in matches]):
            if 'to_k' in name:
                cross_attention_idx += 1
                cross_kv_layer_names.append((cross_attention_idx, 'down_blocks.' + name))
                cross_kv_layer_names.append((cross_attention_idx, 'down_blocks.' + name.replace('to_k', 'to_v')))
            else:
                pass
    ...
    for name, _ in unet.mid_block.named_parameters():
        if any([x in name for x in matches]):
            if 'to_k' in name:
                ...
                cross_kv_layer_names.append((cross_attention_idx, 'mid_block.' + name.replace('to_k', 'to_v')))
            else:
                pass
    ...
    for name, _ in unet.up_blocks.named_parameters():
        if any([x in name for x in matches]):
            if 'to_k' in name:
                ...
                cross_kv_layer_names.append((cross_attention_idx, 'up_blocks.' + name.replace('to_k', 'to_v')))
            else:
                pass
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_crosskv_list):
        concept_prompt = [TEMPLATE_SIMPLE.format(concept['concept_name']), concept['concept_name']]
        concept_prompt = bind_concept_prompt(concept_prompt, new_concept_cfg)

        n = len(concept_prompt) // 16
        layer_prompts = [tuple(concept_prompt[j * 16 + i] for j in range(n)) for i in range(16)]
        ...
        for layer_idx, layer_name in cross_kv_layer_names:
            ...
            lora_down_name = layer_name.replace('to_k.weight',
                                                'to_k_lora.down.weight').replace('to_v.weight', 'to_v_lora.down.weight')
            lora_up_name = lora_down_name.replace('lora.down', 'lora.up')
    ...
    for idx, (layer_idx, layer_name) in enumerate(cross_kv_layer_names):
        ...
        Wnew = update_quasi_newton(
            ...
            new_concept_output,  # our concept
            W.clone(),
...
def merge_text_encoder(concept_list, optimize_iters, new_concept_cfg, tokenizer, text_encoder, text_encoder_list,
                       device):

    def process_extract_features(input_feature_list, output_feature_list):
        text_input_features = [feat.reshape(-1, feat.shape[-1]) for feat in input_feature_list]
        text_output_features = [feat.reshape(-1, feat.shape[-1]) for feat in output_feature_list]
    ...
    candidate_module_name = [name for name in candidate_module_name if any([name in key for key in LoRA_keys])]
    ...
    for concept, lora_state_dict in zip(concept_list, text_encoder_list):
        ...
        module_io_recoder = {}
        record_feature = True
        _ = get_text_feature(concept_prompt, tokenizer, text_encoder, device, return_type='category_embedding')
    ...
    for idx, layer_name in enumerate(text_encoder_layer_names):
        W = original_state_dict[layer_name]
...
@torch.no_grad()
def decode_to_latents(concept_prompt, new_concept_cfg, tokenizer, text_encoder, unet, test_scheduler,
                      num_inference_steps, device, record_nums, batch_size):

    concept_prompt = bind_concept_prompt([concept_prompt], new_concept_cfg)
    text_embeddings = get_text_feature(
        concept_prompt, tokenizer, text_encoder, device, return_type='full_embedding').unsqueeze(0)
    ...
    record_timestep = test_scheduler.timesteps[torch.arange(0, test_scheduler.timesteps.size(0),
                                                            step=step)[:record_nums]]
...
def merge_spatial_attention(concept_list, optimize_iters, new_concept_cfg, tokenizer, text_encoder, unet,
                            unet_spatial_attn_list, test_scheduler, device):
    LoRA_keys = []
    ...
    candidate_module_name = [
        'attn2.to_q', 'attn2.to_out.0', 'attn1.to_q', 'attn1.to_k', 'attn1.to_v', 'attn1.to_out.0', 'ff.net.2',
        'ff.net.0.proj', 'proj_out', 'proj_in'
    ]
    candidate_module_name = [name for name in candidate_module_name if any([name in key for key in LoRA_keys])]

    logger.info(f'unet have {len(spatial_attention_layer_names)} linear layer need to optimize')
    ...
    for idx, layer_name in enumerate(spatial_attention_layer_names):
        ...
        W = original_state_dict[layer_name]  # origin params
        Wnew = update_quasi_newton(
            new_concept_input,  # our concept
            new_concept_output,  # our concept
            W.clone(),
...
def compose_concepts(concept_cfg, optimize_textenc_iters, optimize_unet_iters, pretrained_model_path, save_path, suffix,
                     device):
    logger = get_root_logger()
    ...
    embedding_list, text_encoder_list, unet_crosskv_list, unet_spatial_attn_list, concept_list = parse_new_concepts(
        concept_cfg)
    ...
    if any([item is not None for item in embedding_list]):
        ...
        embedding_features, new_concept_cfg = merge_new_concepts_(embedding_list, concept_list, tokenizer, text_encoder)
    else:
        embedding_features, new_concept_cfg = {}, {}
    ...
    if any([item is not None for item in text_encoder_list]):
        ...
        new_text_encoder_weights = merge_text_encoder(concept_list, optimize_textenc_iters, new_concept_cfg, tokenizer,
    ...
    if any([item is not None for item in unet_crosskv_list]):
        ...
        new_kv_weights = merge_kv_in_cross_attention(concept_list, optimize_textenc_iters, new_concept_cfg, tokenizer,
                                                     text_encoder, unet, unet_crosskv_list, device)
    ...
    if any([item is not None for item in unet_spatial_attn_list]):
        ...
        new_spatial_attention_weights = merge_spatial_attention(concept_list, optimize_unet_iters, new_concept_cfg,
                                                                tokenizer, text_encoder, unet, unet_spatial_attn_list,
                                                                test_scheduler, device)
    else:
        new_spatial_attention_weights = {}
        logger.info('------Step 6: no new spatial-attention in unet, skip merging spatial attention------')

    new_unet_weights = new_kv_weights
    new_unet_weights.update(new_spatial_attention_weights)
